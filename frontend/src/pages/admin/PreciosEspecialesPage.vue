<template>
  <q-page class="q-pa-md">
    <page-title
      ...
      subtitle="Precios personalizados de servicios para agencias específicas"
    />
    ...
    >
      ...
      <template v-slot:body-cell-precio="props">
        <q-td :props="props">
          ...
          <div class="text-caption text-grey-7" v-if="props.row.servicio">
            Precio normal: S/ {{ parseFloat(props.row.servicio.precio || 0).toFixed(2) }}
          </div>
        </q-td>
      </template>
    ...
    <q-dialog v-model="showDialog" persistent :maximized="$q.screen.xs">
      <q-card :style="$q.screen.gt.xs ? 'min-width: 700px; max-width: 800px' : ''">
        ...
        <q-card-section class="q-pa-md">
          <q-form @submit="savePrecio" class="q-gutter-md">
            <div class="row q-col-gutter-md">
              <div class="col-12">
                <div class="text-subtitle2 text-grey-7 q-mb-sm">
                  <q-icon name="info" size="xs" class="q-mr-xs" />
                  Configure un precio especial para un servicio específico de una agencia
                </div>
              </div>
...
const loadPrecios = async (props) => {
  ...
  try {
    ...
    const response = await api.get('base/servicio-precios-especiales/', { params })
...
const openDialog = (precio = null) => {
  if (precio) {
    ...
    form.value = {
      id: precio.id,
      servicio: precio.servicio || { id: precio.servicio, nombre: precio.servicio_nombre },
...
const savePrecio = async () => {
  ...
  try {
    ...
    if (isEditing.value) {
      await api.put(`base/servicio-precios-especiales/${form.value.id}/`, payload)
      notifySuccess('Precio especial actualizado correctamente')
    } else {
      await api.post('base/servicio-precios-especiales/', payload)
...
const deletePrecio = async (precio) => {
  if (
    !(await confirm(
      `¿Eliminar el precio especial de ${precio.servicio_nombre} para ${precio.cliente_nombre}?`,
    ))
  )
    return

  try {
    await api.delete(`base/servicio-precios-especiales/${precio.id}/`)
